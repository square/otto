...
public class Bus {
  ...
  public void register(Object object) {
    ...
    for (Class<?> type : foundPublishers.keySet()) {
      ...
      if (previousPublisher != null) {
        throw new IllegalArgumentException("Publisher method for type " + type
          + " found on type " + publisher.target.getClass()
          + ", but already registered by type " + previousPublisher.target.getClass() + ".");
      }
  ...
  private void dispatchPublisherResultToHandler(EventHandler handler, EventPublisher publisher) {
    ...
    try {
      ...
      throwRuntimeException("Publisher " + publisher + " threw an exception.", e);
    }
    if (event == null) {
      return;
    }
