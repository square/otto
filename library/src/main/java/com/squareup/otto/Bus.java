...
/**
 * Interface for event bus implementations, which dispatch events to listeners and provide ways for
 * listeners to register themselves.
 * <p>
 * A class implementing this interface (henceforth, a "bus") allows publish-subscribe-style
 * communication between components without requiring the components to explicitly register with
 * one another (and thus be aware of each other).  This is designed exclusively to replace
 * traditional Android in-process event distribution using explicit registration or listeners. It
 * is <em>not</em> a general-purpose publish-subscribe system, nor is it intended for interprocess
 * communication.
 *
...
public interface Bus {
   ...
   * immediately with the result of calling that producer, unless the result is null.
   * <p>
